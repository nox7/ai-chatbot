...
class NeuralChunk{
	...
	*/
	getAdjacentChunkPositions(){
		...
		// Coordinates that actually relate to real, existing chunks
		let filteredChunks = [];

		adjacentChunks.forEach(chunkCoordinate => {
			const possibleChunk = this.brain.getChunkFromChunkPosition(chunkCoordinate);
			if (possibleChunk !== undefined){
				filteredChunks.push(possibleChunk);
			}
		});
	...
	/**
	* Gets existing, adjacent chunks - exclusive
	*
	* @return {NeuralChunk[]}
	*/
	getAdjacentChunks(){
		let existingChunkCoordinates = this.getAdjacentChunkPositions();
		let chunks = [];

		existingChunkCoordinates.forEach(chunkCoordinates => {
			chunks.push(this.brain.neuralChunks[chunkCoordinates.x][chunkCoordinates.y][chunkCoordinates.z]);
		});
