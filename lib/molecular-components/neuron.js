/*
	Notes

	Action Potential = The amount of electrical pulse needed to fire a signal along the axon
*/
const Vector3Int = require("../vector3Int");
const Dendrite = require("./dendrite");

class Neuron{

	/**
	* Constructs a default brain cell for a new brain
	*/
	constructor(){
		/** @property Dendrite[] All of the dendrites belonging to this neuron */
		this.dendrites = [];

		/** @property ? The axon of the neuron which sends a signal when the action potential of this neuron is reached because of enough signal, collectively, from along its dendrites */
		this.axon = undefined;

		/** @property {NeuralChunk} chunk The chunk object this neuron belongs to */
		this.chunk = undefined;

		/** @property number The cooldown period (in milliseconds) for which, if no signals were received within the timeframe of this number, the action potential sum should be reset to 0 potential */
		this.actionPotentialCooldown = 0;

		/** @property bool Whether or not the cooldown for the reset timer of the action potential has been started */
		this.actionPotentialCooldownCountdownBegun = false;

		/** @property {string} data The data stored in this neuron. Should mainly just be a character */
		this.data = "";

		/** @property {string} label For front-end use. May be deprecated. Currently used to color a neuron when rendering */
		this.label = "";

		this.x = 0;
		this.y = 0;
		this.z = 0;
	}

	setPosition(x,y,z){
		this.x = x;
		this.y = y;
		this.z = z;
	}

	/**
	* Sets the neuron's position using a Vector3Int object
	*
	* @param {Vector3Int} v3Int
	* @return {undefined}
	*/
	setPositionWithVector3Int(v3Int){
		this.x = v3Int.x;
		this.y = v3Int.y;
		this.z = v3Int.z;
	}

	/**
	* Gets the position as a Vector3Int
	*
	* @return {Vector3Int}
	*/
	getPositionAsVector3Int(){
		return new Vector3Int(
			this.x,
			this.y,
			this.z
		);
	}

	/**
	* Makes a dendrite connection to another neuron
	*
	* @param {Neuron} otherNeuron
	* @return {undefined}
	*/
	connectTo(otherNeuron){
		let dendrite = new Dendrite;
		dendrite.fromNeuron = this;
		dendrite.connection = otherNeuron
		this.dendrites.push(dendrite);
	}

	/**
	* Converts the neuron's data to as primitive as possible for use with JSON encoding
	*
	* @return {object}
	*/
	toJSON(){
		let obj = {};

		obj.x = this.x;
		obj.y = this.y;
		obj.z = this.z;

		if (this.dendrites.length > 0){
			obj.dendrites = this.dendrites;
		}

		if (this.label !== ""){
			obj.label = this.label;
		}

		return obj;
	}
}

module.exports = Neuron;
