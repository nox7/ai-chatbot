/*
	...
	Notes when done for the day

	10/13/2019
	Go through "consensing information" phases which attempts to find node patterns in sentences "stoppings" to formulate what a "word" is. Condense a single-data node into a word node and begin to recognize word patterns and auto-link that pattern to a single, existing word node.

	For instance, if the word "and" comes up from input data stream "a" "n" "d", strip the newly created 'a','n','d' nodes and repoint the traversing system to the "and" node
...
const totalNeurons = 10000;

// The extents of the relative 3D space of the "brain"
// This expands outwards in all directions -spaceExtents to spaceExtents on all 3D axes
const spaceExtents = 420; // This is an exclusive boundary - no neuron will/should ever have the axis coordinate of `spaceExtents`
const numberOfChunksPerAxis = 8;
const chunkSize = Math.ceil( (spaceExtents * 2) / numberOfChunksPerAxis); // spaceExtents is multiplied by 2 here because the negative expansion must be considered

console.log(`Brain will have ${numberOfChunksPerAxis} chunks with each being a uniform size of ${chunkSize}. Total neurons in brain: ${totalNeurons}`);
...
class Brain{
	...
	*/
	generateNeuralChunks(){
		...
		for (let x = 0; x < numberOfChunksPerAxis; ++x){
			for (let y = 0; y < numberOfChunksPerAxis; ++y){
				for (let z = 0; z < numberOfChunksPerAxis; ++z){
					...
					const newChunk = new NeuralChunk(x,y,z);
	...
	generateNeurons(){
		...
		for (let neuronIndex = 0; neuronIndex < totalNeurons; neuronIndex++){
			...
			while (freePositionFound === false){
				randomXInt = Math.ceil(MathLib.getRandomNumber(-spaceExtents+1, spaceExtents-1));
				randomYInt = Math.ceil(MathLib.getRandomNumber(-spaceExtents+1, spaceExtents-1));
				randomZInt = Math.ceil(MathLib.getRandomNumber(-spaceExtents+1, spaceExtents-1));
			...
			const chunkPosition = NeuralChunk.getChunkPositionFromWorldPosition(neuronPosition, chunkSize, spaceExtents);
	...
	findClosestNeuronToPosition(position, ignoreList, onlyFindEmptyNeurons){
		...
		while (lastClosestNeuron === undefined){
			...
			if (optimizedChunkCheckingModeEnable){
				...
				chunkCoordinatesToCheck = NeuralChunk.getChunkPositionsFromPointEdgeCases(position, spaceExtents, chunkSize, 0.2);
			}else{
	...
	* @param {Neuron} fromNeuron
	* @param {string} data
	* @return {Neuron|undefined} Returns undefined if no neuron with that data exists
	*/
	...
	*/
	getChunkFromWorldPosition(worldPosition){
		const chunkCoordinates = NeuralChunk.getChunkPositionFromWorldPosition(worldPosition, chunkSize, spaceExtents);
