...
class Brain{
	...
	*/
	constructor(){
		...
		// TEST
		this.languageNeuron = this.getNeuronAtVector3Int(this.languageQuadrant);
	...
	* @return {Neuron}
	*/
	findClosestNeuron(fromVector3Int, ignoreList, onlyFindEmptyNeurons){
		let closestNeuron;
		...
		for (let i = 0; i < this.neurons.length; i++){
			...
			// Is this neuron non-empty and the call stated only find empty neurons
			if (onlyFindEmptyNeurons === true){
				// The call states it only wants empty neurons
				if (iNeuron.data !== ""){
					// This neuron is not empty, ignore this loop iteration. Move on to the next one
					continue;
				}
			}
