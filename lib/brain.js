...
class Brain{
	...
	*/
	consumeHormones(){
		...
		// NOTE It is difficult to go quickly from good thoughts to bad thoughts without drastic sensory effect (this could be implemented as a side effect of a lot of hormones bound to a receptor - more thoughts related to that hormone)
		...
		LanguageProcessor.process(text, (sentence, noiseLevel) => {
			...
			if (memoriesRelatedToNoise.length === 0){
				...
				if (speakerMood === "happy"){
					aMod += this.dna.hormoneACreationRate;
					xMod += this.dna.hormoneXCreationRate;

					// TODO noise results should be modified by the robot's current mood too
					if (noiseLevel === "loud"){
						aMod *= MathLib.getRandomNumber(1,3);
						cMod += this.dna.hormoneCCreationRate;
					}
				}else if (speakerMood === "neutral"){
					// Neutral, if they robot has bad feelings - slightly lower them

					bMod -= this.dna.hormoneBCreationRate * MathLib.getRandomNumber(0,2);
					cMod -= this.dna.hormoneCCreationRate * MathLib.getRandomNumber(0,4);

					if (noiseLevel === "loud"){
						cMod += this.dna.hormoneCCreationRate * MathLib.getRandomNumber(0,3);
					}
					...
					aMod -= this.dna.hormoneACreationRate;
					bMod += this.dna.hormoneBCreationRate;

					if (noiseLevel === "loud"){
						bMod += this.dna.hormoneBCreationRate;
						cMod += this.dna.hormoneCCreationRate * MathLib.getRandomNumber(1,4);
					}
				}else if (speakerMood === "mad"){
					aMod -= this.dna.hormoneACreationRate * MathLib.getRandomNumber(0,5);
					bMod += this.dna.hormoneBCreationRate;
					cMod += this.dna.hormoneCCreationRate;
				...
				// Experience a "mean" of emotional levels from the memories related to the input
				if (memoriesRelatedToNoise.length > 0){
					let memoryAMod = 0;
					let memoryBMod = 0;
					let memoryCMod = 0;
					...
					memoriesRelatedToNoise.forEach(node => {
						console.log(node);
						memoryAMod += node.hormoneACount;
						memoryBMod += node.hormoneBCount;
						memoryCMod += node.hormoneCCount;
						memoryXMod += node.hormoneXCount;
					});

					aMod += memoryAMod / memoriesRelatedToNoise.length;
					bMod += memoryBMod / memoriesRelatedToNoise.length;
					cMod += memoryCMod / memoriesRelatedToNoise.length;
					xMod += memoryXMod / memoriesRelatedToNoise.length;
				}
				...
				if (speakerMood === "mad"){
					...
					if (aMod >= 0){
						...
						aMod -= this.dna.hormoneACreationRate * MathLib.getRandomNumber(0,5);
						bMod += this.dna.hormoneBCreationRate;
						cMod += this.dna.hormoneCCreationRate * MathLib.getRandomNumber(0,5);
						xMod -= this.dna.hormoneXCreationRate * MathLib.getRandomNumber(0,5);
					...
					if (aMod >= 0){
						...
						aMod -= this.dna.hormoneACreationRate * MathLib.getRandomNumber(0,2);
						bMod += this.dna.hormoneBCreationRate * MathLib.getRandomNumber(1,3);

						if (noiseLevel === "loud"){
							// They shouted their sadness from a once-thought-of happy memory
							aMod -= this.dna.hormoneACreationRate * MathLib.getRandomNumber(2,5);
