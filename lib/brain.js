...
class Brain{
	...
	*/
	constructor(){
		...
		// TEST
		let languageNeuron = this.getNeuronAtVector3Int(this.languageQuadrant);
		let randomNeuron = this.neurons[Math.floor(MathLib.getRandomNumber(0, this.neurons.length))];
		let neuronsToLanguageNeuron = this.findShortestPathToNeuron(randomNeuron, languageNeuron);
		...
		for(let i = 0; i < neuronsToLanguageNeuron.length; i++){
			...
			if (nextNeuron){
				...
				dendrite.connection = languageNeuron;
			}
